      ...
      <main className="pt-24 pb-16">
        <SectionContainer spacing="sm" size="wide">
          <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <h1 className="text-heading font-bold text-text">
                {departmentName ? `Especialistas en ${departmentName}` : 'Directorio Médico'}
              </h1>
          ...
          <div className="flex flex-col gap-8 lg:flex-row">
            <aside className="shrink-0 lg:w-72" aria-label="Filtros de búsqueda">
                  ...
                  <fieldset>
                    ...
                    <div className="mt-2 flex flex-col gap-1">
                      {[0, 3, 3.5, 4, 4.5].map((r) => (
                        <label
                          key={r}
                          className={`flex cursor-pointer items-center gap-3 rounded-[var(--radius-button)] px-3 py-2 text-sm transition-colors ${
                            minRating === r ? 'bg-primary/10 text-primary' : 'text-text-muted hover:bg-secondary'
                          }`}
                        >
                          <input
                            type="radio"
            ...
            <div className="flex-1">
              {isLoading ? (
                ...
                <div className="rounded-[var(--radius-card)] border border-border bg-white py-20 text-center shadow-sm">
                  <p className="text-5xl" aria-hidden="true">🔍</p>
                  <h2 className="mt-4 text-lg font-semibold text-text">Sin resultados</h2>
                  <p className="mx-auto mt-2 max-w-md text-sm text-text-muted">
                    No encontramos especialistas con los filtros seleccionados. Intenta ajustar tu búsqueda.
                  </p>
