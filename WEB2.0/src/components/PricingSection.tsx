  ...
  return (
    <li
      className={`flex items-start gap-3 text-sm ${
        !feature.included && !isPopular ? 'text-text-muted/50 line-through' : ''
      }`}
    >
      {feature.included ? (
        <Check
...
export default function PricingSection() {
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => setCardsVisible(true), 100);
    ...
    >
      <div className="mx-auto mb-14 max-w-2xl text-center">
        ...
        <p className="text-subheading mt-4 text-text-muted">
          Desde consultorios individuales hasta centros médicos. Escala tu presencia digital con confianza.
        </p>
        ...
        >
          <button
            ...
            className={`rounded-[var(--radius-pill)] px-5 py-2 text-sm font-semibold transition-all duration-300 transition-premium ${
              !isYearly ? 'bg-primary text-white shadow-md' : 'text-text-muted hover:text-text'
            }`}
          >
            Mensual
          </button>
          <button
            ...
            className={`flex items-center gap-2 rounded-[var(--radius-pill)] px-5 py-2 text-sm font-semibold transition-all duration-300 transition-premium ${
              isYearly ? 'bg-primary text-white shadow-md' : 'text-text-muted hover:text-text'
            }`}
          >
            Anual
            ...
            >
              ...
              >
                {isPopular && (
                  <Badge
                    variant="accent"
                    className="absolute -top-3 left-1/2 -translate-x-1/2 animate-pulse-slow shadow-md"
                  >
                    Most Popular
                  </Badge>
                )}
                  ...
                  <div className="flex items-baseline gap-1">
                    <span className="text-4xl font-extrabold tracking-tight">
                      ...
                      {price === 0 ? (
                        ...
                        <AnimatedPrice value={price} isVisible={cardsVisible} />
                      )}
                    </span>
